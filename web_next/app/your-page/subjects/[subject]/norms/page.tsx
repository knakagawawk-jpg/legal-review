...
function NormsPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const normsData = await apiClient.get<StudyItemApi[]>(`/api/study-items?subject_id=${subjectId}&entry_type=1`)
      setAllItems((Array.isArray(normsData) ? normsData : []).map(toUiStudyItem))
    } catch (error) {
        ...
        <div className="container mx-auto px-20 py-3 max-w-6xl">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-base font-semibold text-amber-900">規範一覧</h1>
            </div>
            <Button
              variant="outline"
              size="sm"
                          ...
                          {displayedItems.map((item) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <TableCell className="py-2 px-2 w-[92px] align-top">
                                  <Select
                                ...
                                <TableCell className="py-2 px-2 w-[140px] align-top">
                                  <Popover
                                          ...
                                          onKeyDown={(e) => {
                                            if (e.key === 'Enter') {
                                              ...
                                              if (newTag && !normTags.includes(newTag)) {
                                                ...
                                                const updatedItems = allItems.map(n =>
                                                  n.id === item.id ? { ...n, tags: [...normTags, newTag] } : n
                                                )
                                                setAllItems(updatedItems)
                                                queueUpdateStudyItem(item.id, { tags: [...normTags, newTag] })
                                                input.value = ""
                                              }
                                            }
                                          }}
                                  ...
                                  >
                                    ...
                                    <PopoverContent className="w-auto p-3" align="start">
                                      <DatePickerCalendar
                                        ...
                                        onSelect={(date) => {
                                          if (date) {
                                            updateCreatedDate(item.id, date, "norms")
...
export default withAuth(NormsPage, { requireAuth: true })
