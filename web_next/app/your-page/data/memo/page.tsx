...
import { History, BookOpen, ChevronDown, Filter, Menu, Lightbulb, Heart, Calendar as CalendarIcon, ArrowLeft, Plus, CalendarDays } from "lucide-react"
...
function MemoPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true)
      const data = await apiClient.get<{ items: DashboardItem[], total: number }>("/api/dashboard/items/all?entry_type=1")
  ...
  const convertToTopic = async (itemId: number) => {
    try {
      await apiClient.put(`/api/dashboard/items/${itemId}`, { entry_type: 2 })
      loadData()
    } catch (error) {
      console.error("Failed to convert to topic:", error)
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-base font-semibold text-amber-900">Your MEMO</h1>
      </div>
            <Button
              variant="outline"
              size="sm"
        ...
        <div className="mb-4 flex flex-col gap-2 md:flex-row md:flex-wrap md:items-center">
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="all" className="text-xs"><span style={{ fontSize: '10px' }}>フィルターなし</span></SelectItem>
            </SelectContent>
          </Select>
          </div>
              ...
              <>
                ...
                >
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm min-w-[589px]">
                      <thead>
                        <tr className="border-b border-border text-xs text-muted-foreground">
                          ...
                          <TableHead className="py-2 px-2 w-20 text-left font-medium">種類</TableHead>
                          ...
                          <TableHead className="py-2 px-2 w-20 text-left font-medium">作成日</TableHead>
                        </tr>
                      </thead>
                      <SortableContext items={displayedItems.map(p => p.id.toString())} strategy={verticalListSortingStrategy}>
                        ...
                        return (
                          <SortableRow 
                            ...
                            entryType={1}
                            onConvertToTopic={convertToTopic}
                            itemText={item.item}
                          >
...
export default withAuth(MemoPage, { requireAuth: true })
