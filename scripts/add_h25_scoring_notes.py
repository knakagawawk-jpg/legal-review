...
# -*- coding: utf-8 -*-
"""
H25司法試験JSONに採点実感PDFから抽出したscoring_notesを追加するスクリプト

採点実感.pdfの構造:
- 平成２５年司法試験の採点実感等に関する意見（公法系科目第１問）→ 憲法
- 平成２５年司法試験の採点実感等に関する意見（公法系科目第２問）→ 行政法
- 民事系科目第１問 → 民法
- 民事系科目第２問 → 商法
- 民事系科目第３問 → 民事訴訟法
- 刑事系科目第１問 → 刑法
...
PDF_PATH = BASE_DIR / "data" / "pdfs" / "judicial_exam" / "H25" / "論述" / "採点実感.pdf"
H25_DIR = BASE_DIR / "data" / "json" / "judicial_exam" / "H25"
...
def extract_scoring_notes_from_pdf(pdf_path: Path) -> dict[str, str]:
    ...
    # 見出しパターン: 平成２５年司法試験の採点実感等に関する意見（XXX）
    pattern = re.compile(
        r"平成[２５25]+年司法試験の採点実感等に関する意見\s*[（(]\s*([^）)\n]+)[）)]",
        re.MULTILINE,
    )
...
def add_scoring_notes_to_json(h25_dir: Path, notes_by_subject: dict[str, str]) -> None:
    ...
    for json_path in sorted(h25_dir.glob("H25_司法_*.json")):
        stem = json_path.stem
        subject_name = stem.replace("H25_司法_", "")
...
def main():
    ...
    add_scoring_notes_to_json(H25_DIR, notes_by_subject)
    print("完了しました。")
