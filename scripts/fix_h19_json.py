...
H19_DIR = Path(__file__).parent.parent / "data" / "json" / "judicial_exam" / "H19"
...
def main():
    ...
    if civil_path.exists():
        ...
        with open(H19_DIR / "H19_司法_民法.json", "w", encoding="utf-8") as f:
            json.dump(civil_d, f, ensure_ascii=False, indent=2)
        with open(H19_DIR / "H19_司法_商法.json", "w", encoding="utf-8") as f:
            json.dump(commercial_d, f, ensure_ascii=False, indent=2)
        with open(H19_DIR / "H19_司法_民事訴訟法.json", "w", encoding="utf-8") as f:
            json.dump(minshu_d, f, ensure_ascii=False, indent=2)
        civil_path.unlink()
        print("民事系 → 民法, 商法, 民事訴訟法 に分割しました")

    # 3. 刑事系を分割
    criminal_path = H19_DIR / "H19_司法_刑事系.json"
    if criminal_path.exists():
        ...
        with open(H19_DIR / "H19_司法_刑法.json", "w", encoding="utf-8") as f:
            json.dump(kei_d, f, ensure_ascii=False, indent=2)
        with open(H19_DIR / "H19_司法_刑事訴訟法.json", "w", encoding="utf-8") as f:
            json.dump(keiji_d, f, ensure_ascii=False, indent=2)
        criminal_path.unlink()
        print("刑事系 → 刑法, 刑事訴訟法 に分割しました")

    # 4. 残りのH19ファイルのtext/purposeをクリーニング
    for f in H19_DIR.glob("*.json"):
        process_file(f)
