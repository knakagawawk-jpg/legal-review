...
H20_DIR = Path(__file__).parent.parent / "data" / "json" / "judicial_exam" / "H20"
...
def split_civil_law(data: dict) -> tuple[dict, dict, dict]:
    ...
    minshu_text = (
        ...
        if q3_start > 0
        else clean_text_simple(text[q2_start:].strip()) if q2_start >= 0 else ""  # 第3問がなければ第2問（融合）を民訴にも
    )
    ...
    minshu_purpose = (
        ...
        if purpose_q3 > 0
        else clean_text_simple(civil_purpose_block[purpose_q2:].strip()) if purpose_q2 >= 0 else ""  # 第3問がなければ第2問を民訴にも
    )
...
def main():
    H20_DIR.mkdir(parents=True, exist_ok=True)

    # 1. 公法系を分割
    public_path = H20_DIR / "H20_司法_公法系.json"
    if public_path.exists():
        with open(public_path, "r", encoding="utf-8") as f:
            public_data = json.load(f)
        const_data, admin_data = split_public_law(public_data)
        with open(H20_DIR / "H20_司法_憲法.json", "w", encoding="utf-8") as f:
            json.dump(const_data, f, ensure_ascii=False, indent=2)
        with open(H20_DIR / "H20_司法_行政法.json", "w", encoding="utf-8") as f:
            json.dump(admin_data, f, ensure_ascii=False, indent=2)
        public_path.unlink()
        print("公法系 → 憲法, 行政法 に分割しました")

    # 2. 民事系を分割
    civil_path = H20_DIR / "H20_司法_民事系.json"
    if civil_path.exists():
        ...
        with open(H20_DIR / "H20_司法_民法.json", "w", encoding="utf-8") as f:
            json.dump(civil_d, f, ensure_ascii=False, indent=2)
        with open(H20_DIR / "H20_司法_商法.json", "w", encoding="utf-8") as f:
            json.dump(commercial_d, f, ensure_ascii=False, indent=2)
        with open(H20_DIR / "H20_司法_民事訴訟法.json", "w", encoding="utf-8") as f:
            json.dump(minshu_d, f, ensure_ascii=False, indent=2)
        civil_path.unlink()
        print("民事系 → 民法, 商法, 民事訴訟法 に分割しました")

    # 3. 刑事系を分割
    criminal_path = H20_DIR / "H20_司法_刑事系.json"
    if criminal_path.exists():
        ...
        with open(H20_DIR / "H20_司法_刑法.json", "w", encoding="utf-8") as f:
            json.dump(kei_d, f, ensure_ascii=False, indent=2)
        with open(H20_DIR / "H20_司法_刑事訴訟法.json", "w", encoding="utf-8") as f:
            json.dump(keiji_d, f, ensure_ascii=False, indent=2)
        criminal_path.unlink()
        print("刑事系 → 刑法, 刑事訴訟法 に分割しました")

    # 4. 残りのH20ファイルのtext/purposeをクリーニング
    for f in H20_DIR.glob("*.json"):
        process_file(f)
