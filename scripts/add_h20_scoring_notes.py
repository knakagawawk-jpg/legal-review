...
# -*- coding: utf-8 -*-
"""
H20司法試験JSONに採点実感PDFから抽出したscoring_notesを追加するスクリプト

採点実感.pdfの構造:
- 平成２０年司法試験の採点実感等に関する意見（公法系科目第１問）→ 憲法
- 平成２０年司法試験の採点実感等に関する意見（公法系科目第２問）→ 行政法
- 民事系科目第１問 → 民法
- 民事系科目第２問 → 商法
- 民事系科目第３問 → 民事訴訟法
- 刑事系科目第１問 → 刑法
...
PDF_PATH = BASE_DIR / "data" / "pdfs" / "judicial_exam" / "H20" / "論述" / "採点実感.pdf"
H20_DIR = BASE_DIR / "data" / "json" / "judicial_exam" / "H20"

# PDF見出し → JSONのsubject_name（複数科目に紐づく場合あり）
# H20の採点実感は「憲法」「行政法」等の単純な科目名で記載される場合がある
HEADER_TO_SUBJECTS = {
    # 公法系・民事系・刑事系（第○問形式）
    "公法系科目第１問": ["憲法"],
    "公法系科目第２問": ["行政法"],
...
def extract_scoring_notes_from_pdf(pdf_path: Path) -> dict[str, str]:
    ...
    # 見出しパターン: 平成２０年司法試験の採点実感等に関する意見（XXX）
    # 全角・半角数字、全角・半角括弧、改行の混入に対応
    pattern = re.compile(
        r"平成[２０20]+年司法試験の採点実感等に関する意見\s*[（(]\s*([^）)\n]+)[）)]",
        re.MULTILINE,
    )
    matches = list(pattern.finditer(full_text))

    # 別パターン: より簡易な形式（「意見」の直後の括弧）
    if not matches:
    ...
    for i, match in enumerate(matches):
        ...
        # 見出し文字列もキーとして保存（別表記でのルックアップ用）
        notes_by_subject[header] = cleaned

    return notes_by_subject


def add_scoring_notes_to_json(h20_dir: Path, notes_by_subject: dict[str, str]) -> None:
    """各JSONにscoring_notesを追加"""
    # 科目名の別表記マッピング（ファイル名ベース → PDF見出しで使われる表記）
    subject_aliases = {
        "国際関係法_公法系_": ["国際関係法（公法系）"],
        "国際関係法_私法系_": ["国際関係法（私法系）"],
    }

    for json_path in sorted(h20_dir.glob("H20_司法_*.json")):
        stem = json_path.stem
        subject_name = stem.replace("H20_司法_", "")
...
def main():
    ...
    add_scoring_notes_to_json(H20_DIR, notes_by_subject)
    print("完了しました。")
