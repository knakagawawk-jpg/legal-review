...
# -*- coding: utf-8 -*-
"""
司法試験JSON修正スクリプト（H26～R7対応）
- 公法系、民事系、刑事系を個別科目に分割
- TextとPurposeからPDF由来の不要な改行を削除

使用例: python fix_judicial_json.py H26 H27 H28 ...
引数なしの場合は H26, H27, H28, H29, H30, R1, R2, R3, R4, R5, R6, R7 を処理
"""
...
def split_public_law(data: dict, year_prefix: str) -> tuple[dict, dict]:
    """公法系を憲法と行政法に分割"""
    text = data["text"]
    ...
    if purpose_civil < 0:
        purpose_civil = purpose.find("【民事系科目】")
...
def split_civil_law(data: dict, year_prefix: str) -> tuple[dict, dict, dict]:
    """民事系を民法、商法、民事訴訟法に分割"""
    text = data["text"]
    purpose = data.get("purpose", "")
    ...
    if purpose_criminal < 0:
        purpose_criminal = purpose.find("【刑事系科目】")
    ...
    civil = {"year": data["year"], "exam_type": data["exam_type"], "subject_name": "民法", "text": civil_text, "source_pdf": data["source_pdf"], "purpose": civil_purpose}
    commercial = {"year": data["year"], "exam_type": data["exam_type"], "subject_name": "商法", "text": commercial_text, "source_pdf": data["source_pdf"], "purpose": commercial_purpose}
    minshu = {"year": data["year"], "exam_type": data["exam_type"], "subject_name": "民事訴訟法", "text": minshu_text, "source_pdf": data["source_pdf"], "purpose": minshu_purpose}
    return civil, commercial, minshu


def split_criminal_law(data: dict, year_prefix: str) -> tuple[dict, dict]:
    """刑事系を刑法と刑事訴訟法に分割"""
    text = data["text"]
    purpose = data.get("purpose", "")
    ...
    if purpose_select < 0:
        purpose_select = purpose.find("【選択科目】")
    ...
    kei = {"year": data["year"], "exam_type": data["exam_type"], "subject_name": "刑法", "text": kei_text, "source_pdf": data["source_pdf"], "purpose": kei_purpose}
    keiji = {"year": data["year"], "exam_type": data["exam_type"], "subject_name": "刑事訴訟法", "text": keiji_text, "source_pdf": data["source_pdf"], "purpose": keiji_purpose}
    return kei, keiji


def process_other_file(filepath: Path) -> bool:
...
def process_year(year: str) -> None:
    """指定年度を処理"""
    dir_path = JSON_BASE / year
    if not dir_path.exists():
        print(f"  {year}: ディレクトリなし")
        return

    prefix = f"{year}_司法_"
    modified = False

    public_path = dir_path / f"{prefix}公法系.json"
    if public_path.exists():
        with open(public_path, "r", encoding="utf-8") as f:
            public_data = json.load(f)
        const_data, admin_data = split_public_law(public_data, prefix)
        with open(dir_path / f"{prefix}憲法.json", "w", encoding="utf-8") as f:
            json.dump(const_data, f, ensure_ascii=False, indent=2)
        with open(dir_path / f"{prefix}行政法.json", "w", encoding="utf-8") as f:
            json.dump(admin_data, f, ensure_ascii=False, indent=2)
        public_path.unlink()
        print(f"  {year}: 公法系 → 憲法, 行政法")
        modified = True

    civil_path = dir_path / f"{prefix}民事系.json"
    if civil_path.exists():
        with open(civil_path, "r", encoding="utf-8") as f:
            civil_data = json.load(f)
        civil_d, commercial_d, minshu_d = split_civil_law(civil_data, prefix)
        with open(dir_path / f"{prefix}民法.json", "w", encoding="utf-8") as f:
            json.dump(civil_d, f, ensure_ascii=False, indent=2)
        with open(dir_path / f"{prefix}商法.json", "w", encoding="utf-8") as f:
            json.dump(commercial_d, f, ensure_ascii=False, indent=2)
        with open(dir_path / f"{prefix}民事訴訟法.json", "w", encoding="utf-8") as f:
            json.dump(minshu_d, f, ensure_ascii=False, indent=2)
        civil_path.unlink()
        print(f"  {year}: 民事系 → 民法, 商法, 民事訴訟法")
        modified = True

    criminal_path = dir_path / f"{prefix}刑事系.json"
    if criminal_path.exists():
        with open(criminal_path, "r", encoding="utf-8") as f:
            criminal_data = json.load(f)
        kei_d, keiji_d = split_criminal_law(criminal_data, prefix)
        with open(dir_path / f"{prefix}刑法.json", "w", encoding="utf-8") as f:
            json.dump(kei_d, f, ensure_ascii=False, indent=2)
        with open(dir_path / f"{prefix}刑事訴訟法.json", "w", encoding="utf-8") as f:
            json.dump(keiji_d, f, ensure_ascii=False, indent=2)
        criminal_path.unlink()
        print(f"  {year}: 刑事系 → 刑法, 刑事訴訟法")
        modified = True
...
def main():
    ...
    print("fix_judicial_json: ", ", ".join(years))
    for year in years:
        process_year(year)
