...
# -*- coding: utf-8 -*-
"""
H23司法試験JSONに採点実感PDFから抽出したscoring_notesを追加するスクリプト

採点実感.pdfの構造:
- 平成２３年司法試験の採点実感等に関する意見（公法系科目第１問）→ 憲法
- 平成２３年司法試験の採点実感等に関する意見（公法系科目第２問）→ 行政法
- 民事系科目第１問 → 民法
- 民事系科目第２問 → 商法
- 民事系科目第３問 → 民事訴訟法
- 刑事系科目第１問 → 刑法
...
PDF_PATH = BASE_DIR / "data" / "pdfs" / "judicial_exam" / "H23" / "論述" / "採点実感.pdf"
H23_DIR = BASE_DIR / "data" / "json" / "judicial_exam" / "H23"
...
def extract_scoring_notes_from_pdf(pdf_path: Path) -> dict[str, str]:
    ...
    # 見出しパターン: 平成２３年司法試験の採点実感等に関する意見（XXX）
    # 全角・半角数字に対応（23, ２３等）
    pattern = re.compile(
        r"平成[２３23]+年司法試験の採点実感等に関する意見\s*[（(]\s*([^）)\n]+)[）)]",
        re.MULTILINE,
    )
...
def add_scoring_notes_to_json(h23_dir: Path, notes_by_subject: dict[str, str]) -> None:
    ...
    for json_path in sorted(h23_dir.glob("H23_司法_*.json")):
        stem = json_path.stem
        subject_name = stem.replace("H23_司法_", "")
...
def main():
    ...
    add_scoring_notes_to_json(H23_DIR, notes_by_subject)
    print("完了しました。")
