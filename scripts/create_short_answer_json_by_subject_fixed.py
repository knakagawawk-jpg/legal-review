...
def parse_answer_pdf(answer_text: str) -> Dict[int, Dict]:
    ...
    for line in lines:
        line = line.strip()
        if not line or ("問" in line and "No" in line and "解答" in line):
            continue
...
def parse_problem_text(text: str) -> List[Dict]:
    ...
    # 正しい科目マーカー（「［憲法］」「［行政法］」など）
    subject_pattern = re.compile(r'［(憲法|行政法|刑法|刑事訴訟法|民法|商法|民事訴訟法)］')
    
    problem_pattern = re.compile(r'〔第\s*(\d+)\s*問〕')
    ...
    while i < len(lines):
        ...
        if problem_match:
            ...
            # 新しい問題を開始
            current_problem_num = int(problem_match.group(1))
            current_question_text = ""
            current_choices = []
            
            remaining = line[problem_match.end():].strip()
            if remaining:
                current_question_text += remaining + "\n"
        
        elif current_problem_num is not None:
            # 選択肢を探す
            ...
            if choice_match:
                ...
                if next_problem_match:
                    ...
                    current_problem_num = int(next_problem_match.group(1))
                    current_question_text = ""
                    current_choices = []
...
def process_short_answer_pdf(problem_pdf_path: Path, answer_pdf_path: Path, 
    ...
    for problem in problems:
        ...
        if not subject:
            # 科目が判定できない場合は、元のファイル名から推測
            if "憲法" in problem_pdf_path.name and "行政法" in problem_pdf_path.name:
                # 問題番号から推測（暫定的）
                if problem_num <= 12:
                    subject = "憲法"
                else:
