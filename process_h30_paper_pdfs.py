...
# -*- coding: utf-8 -*-
"""
H30予備試験の論文PDFファイルからテキストを抽出し、科目ごとにJSONファイルとして保存
...
def extract_purpose_by_subjects(purpose_pdf_path: Path) -> dict[str, str]:
    ...
    for subject_name, subject_text in subjects:
        ...
        purpose_only_text = cleaned_text
        for pattern in purpose_patterns:
            purpose_match = re.search(pattern, purpose_only_text)
            if purpose_match:
                # 「（出題の趣旨）」の後のテキストを取得
                purpose_only_text = purpose_only_text[purpose_match.end():].strip()
                # 次の科目の見出し（「［」で始まる）までを取得
                next_subject_match = re.search(r'^［', purpose_only_text, flags=re.MULTILINE)
                if next_subject_match:
                    purpose_only_text = purpose_only_text[:next_subject_match.start()].strip()
                break
        
        # 実務基礎科目の名前をR7の形式に統一
        normalized_subject = normalize_subject_name(subject_name)
        
        purpose_dict[normalized_subject] = purpose_only_text
        print(f"    科目: {normalized_subject} ({len(purpose_only_text)} 文字)")
...
def normalize_subject_name(subject_name: str) -> str:
    ...
    mapping = {
        ...
        '法律実務基礎科目(民事)': '実務基礎（民事）',
        '法律実務基礎科目(刑事)': '実務基礎（刑事）',
        '法律実務基礎科目(民事・刑事)': '実務基礎（民事）',  # 複合科目の場合は民事として扱う（後で分割）
        '民事': '実務基礎（民事）',
        '刑事': '実務基礎（刑事）',
    }
...
def process_paper_pdfs():
    ...
    # 出題趣旨PDFを処理
    purpose_pdf = PDF_DIR / "H30_予備_論文_出題趣旨.pdf"
    purpose_dict = {}
        ...
        for subject_name, subject_text in subjects:
            ...
            if '・' in subject_name:
                ...
                for part in parts:
                    ...
                    # この部分のテキストを抽出（簡易的に全体テキストを使用）
                    # 実際には、より精密な分割が必要な場合がある
                    part_text = cleaned_text  # 簡易的に全体を使用
                    
                    # 出題趣旨を取得
                    purpose = find_purpose_for_subject(normalized_part, purpose_dict)
                    
                    if purpose:
                        print(f"      {normalized_part}: 出題趣旨を追加 ({len(purpose)} 文字)")
                    else:
                ...
                # 複合科目の処理が完了したので、次の科目へ
                continue
            
            # 実務基礎科目の複合（「法律実務基礎科目（民事・刑事）」など）を分割
            if '法律実務基礎科目' in subject_name and ('民事' in subject_name or '刑事' in subject_name):
                # 民事と刑事を個別に処理
            ...
            # 単一科目の場合
            # 出題趣旨を取得
            purpose = find_purpose_for_subject(normalized_subject, purpose_dict)
